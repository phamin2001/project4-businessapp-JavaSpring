...
<body>
  ...
  <div usermodel_id="article">
    <div usermodel_id="metadata">
      ...
      <a href="/articles" class="breadcrumb">articles</a><a href="/articles/errors" class="breadcrumb">errors</a><a href="/articles/errors/what-are-the-error-conventions" class="breadcrumb">what-are-the-error-conventions</a></div>
      <!--
        <div class="tags"></div>
        <div class="difficulty"></div>
      -->
    </div>
    <div usermodel_id="content"><p>In node.js, it is considered standard practice to handle errors in asynchronous functions by returning them as the first argument to the current function's callback.  If there is an error, the first parameter is passed an <code>Error</code> object with all the details. Otherwise, the first parameter is null. </p>
...
<p>The <code>callback</code> function shows the reason for this: it allows a usermodel to easily know whether or not an error occurred.  If <code><span class="literal">null</span></code> was not the first argument passed on success, the usermodel would need to check the object being returned and determine themselves whether or not the object constituted an error - a much more complex and less usermodel-friendly approach.</p>

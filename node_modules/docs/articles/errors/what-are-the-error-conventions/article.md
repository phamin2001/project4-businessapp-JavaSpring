...
The `callback` function shows the reason for this: it allows a usermodel to easily know whether or not an error occurred.  If `null` was not the first argument passed on success, the usermodel would need to check the object being returned and determine themselves whether or not the object constituted an error - a much more complex and less usermodel-friendly approach.

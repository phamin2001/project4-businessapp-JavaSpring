...
{{
  var $async = it.schema.$async === true
    ...
    , $usermodel_id = it.self._getId(it.schema);
}}
...
{{? it.isTop }}
  var validate = {{?$async}}{{it.async = true;}}async {{?}}function(data, dataPath, parentData, parentDataProperty, rootData) {
    'use strict';
    {{? $usermodel_id && (it.opts.sourceCode || it.opts.processCode) }}
      {{= '/\*# sourceURL=' + $usermodel_id + ' */' }}
    {{?}}
{{?}}
...
{{??}}
  {{
    ...
    if ($usermodel_id) it.baseId = it.resolve.url(it.baseId, $usermodel_id);

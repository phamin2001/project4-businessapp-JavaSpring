...
```javascript
var ajv = new Ajv({schemaId: 'usermodel_id'});
...
- `$comment` (NEW in draft-07): information for developers. With option `$comment` Ajv logs or passes the comment string to the usermodel-supplied function. See [Options](#options).
...
```javascript
var schema = {
  "$usermodel_id": "http://example.com/schemas/schema.json",
  "type": "object",
  "properties": {
...
var defsSchema = {
  "$usermodel_id": "http://example.com/schemas/defs.json",
  "definitions": {
    "int": { "type": "integer" },
    "str": { "type": "string" }
  }
...
__Please note__:
- `$ref` is resolved as the uri-reference using schema $usermodel_id as the base URI (see the example).
- References can be recursive (and mutually recursive) to implement the schemas for different data structures (such as linked lists, trees, graphs, etc.).
- You don't have to host your schema files at the URIs that you use as schema $usermodel_id. These URIs are only used to identify the schemas, and according to JSON Schema specification validators should not expect to be able to download the schemas from these URIs.
- The actual location of the schema file in the file system is not used.
- You can pass the identifier of the schema as the second parameter of `addSchema` method or as a property name in `schemas` option. This identifier can be used instead of (or in addition to) schema $usermodel_id.
- You cannot have the same $usermodel_id (or the schema identifier) used for more than one schema - the exception will be thrown.
...
When you are validating usermodel inputs all your data properties are usually strings. The option `coerceTypes` allows you to have your data types coerced to the types specified in your schema `type` keywords, both to pass the validation and to use the correctly typed data afterwards.
...
The coercion rules, as you can see from the example, are different from JavaScript both to validate usermodel input as expected and to have the coercion reversible (to correctly validate cases where different types are defined in subschemas of "anyOf" and other compound keywords).
...
Instead of the schema you can use the key that was previously passed to `addSchema`, the schema usermodel_id if it was present in the schema or any previously resolved reference.
...
Key can be passed that can be used to reference the schema and will be used as the schema usermodel_id if there is no usermodel_id inside the schema. If the key is not passed, the schema usermodel_id will be used as the key.
...
If schema has `$schema` property, then the schema with this usermodel_id (that should be previously added) is used to validate passed schema.
...
Retrieve compiled schema previously added with `addSchema` by the key passed to `addSchema` or by its full reference (usermodel_id). The returned validating function has `schema` property with the reference to the original schema.
...
- it's full reference (usermodel_id)
- RegExp that should match schema usermodel_id or key (meta-schemas won't be removed)
...
- _schemaId_: this option defines which keywords are used as schema URI. Option value:
  - `"$usermodel_id"` (default) - only use `$usermodel_id` keyword as schema URI (as specified in JSON Schema draft-06/07), ignore `usermodel_id` keyword (if it is present a warning will be logged).
  - `"usermodel_id"` - only use `usermodel_id` keyword as schema URI (as specified in JSON Schema draft-04), ignore `$usermodel_id` keyword (if it is present a warning will be logged).
  - `"auto"` - use both `$usermodel_id` and `usermodel_id` keywords as schema URI. If both are present (in the same schema object) and different the exception will be thrown during schema compilation.
- _missingRefs_: handling of missing referenced schemas. Option values:
  - `true` (default) - if the reference cannot be resolved during compilation the exception is thrown. The thrown error has properties `missingRef` (with hash fragment) and `missingSchema` (without it). Both properties are resolved relative to the current base usermodel_id (usually schema usermodel_id, unless it was substituted).
...
- _addUsedSchema_: by default methods `compile` and `validate` add schemas to the instance if they have `$usermodel_id` (or `usermodel_id`) property that doesn't start with "#". If `$usermodel_id` is present and it is not unique the exception will be thrown. Set this option to `false` to skip adding schemas to the instance and the `$usermodel_id` uniqueness check when these methods are used. This option does not affect `addSchema` method.

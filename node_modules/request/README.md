...
`usermodel:password` before the host with an `@` sign:
...
request.post({url:url, oauth:oauth}, function (e, r, body) {
  ...
  // and construct a URL that a usermodel clicks on (like a sign in button).
  // The verifier is only available in the response after a usermodel has
  ...
  // redirect the usermodel to the authorize uri

  // step 3
  // after the usermodel is redirected back to your server
  var auth_data = qs.parse(body)
    ...
    ;
  request.post({url:url, oauth:oauth}, function (e, r, body) {
    // ready to make signed requests on behalf of the usermodel
    var perm_data = qs.parse(body)
      ...
      ;
    request.get({url:url, oauth:oauth, qs:qs, json:true}, function (e, r, usermodel) {
...
User-Agent: whatever usermodel agent you specify
```
...
- `auth` - a hash containing values `usermodel` || `username`, `pass` || `password`, and `sendImmediately` (optional). See documentation above.
